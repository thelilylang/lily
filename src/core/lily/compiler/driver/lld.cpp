...
static bool
isPETargetName(StringRef s)
{
    return s == "i386pe" || s == "i386pep" || s == "thumb2pe" || s == "arm64pe";
}

static bool
isPETarget(std::vector<const char *> &v)
{
...
static Flavor
parseFlavor(std::vector<const char *> &v)
{
    // Parse -flavor option.
        ...
        bool exitEarly = true)
{
    ...
    // Delete the global context and clear the global context pointer, so that
                 ...
                 llvm::raw_ostream &stderrOS)
{
    int r = 0;
    {
        ...
        if (!crc.RunSafely([&]() {
                r =
                  lldMain(argc, argv, stdoutOS, stderrOS, /*exitEarly=*/false);
            }))
...
int
lld_main(int argc, char **argv)
{
    InitLLVM x(argc, argv);
    sys::Process::UseANSIEscapeCodes(true);

    if (::getenv("FORCE_LLD_DIAGNOSTICS_CRASH")) {
        llvm::errs() << "crashing due to environment variable "
    ...
    if (!inTestVerbosity())
        return lldMain(
          argc, const_cast<const char **>(argv), llvm::outs(), llvm::errs());
    ...
    for (unsigned i = inTestVerbosity(); i > 0; --i) {
        ...
        if (!mainRet) {
            ...
            // Exit now, to fail the tests if the result is different between
            // runs.
            return r.ret;
        }
    }
...
run__LilyLLD(int argc, const char **argv)
{
    safeLldMain(argc, argv, llvm::outs(), llvm::errs());
}
